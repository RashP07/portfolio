...
const Education = () => {
  ...
  return (
    <div name="Education" className='pt-14 w-full h-auto text-white
    w-full h-full'>
        ...
        <ol className="relative border-s border-gray-200 dark:border-gray-700">
          {educationDetails.map((education) => (
            <li key={education.id} className="mb-10 ms-4">
              ...
              <h3 className="md:text-2xl font-semibold text-[#d4d4d8] ">{education.title}</h3>
              <p className="mb-4 text-base font-normal text-gray-500">{education.location}</p>
                </li>
          ))}
        </ol>
      </div>
